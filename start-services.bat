...
REM Step 1: Start MySQL containers
...
REM Check if databases are accessible
echo 🔍 Checking database connections...
docker exec mysql-product-service mysql -u product_user -pproduct_pass -e "SELECT 1;" > nul 2>&1
if %errorlevel% equ 0 (
...
docker exec mysql-order-service mysql -u order_user -porder_pass -e "SELECT 1;" > nul 2>&1
if %errorlevel% equ 0 (
